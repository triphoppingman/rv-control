...
class ShuntClient(BaseClient):
    def __init__(self, config: Any, on_data_callback: Any = None, on_error_callback: Any = None) -> None:
        """Initialize the Renogy shunt client and its parser definitions."""
    ...
    async def on_data_received(self, response: bytes | bytearray) -> None:
        """Parse shunt response bytes and dispatch the completed reading."""
    ...
    def parse_shunt_info(self, bs: bytes | bytearray) -> dict[str, Any]:
        """Parse shunt voltage, current, power, and state from response bytes."""
        data = {}
