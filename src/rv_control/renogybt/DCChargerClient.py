...
class DCChargerClient(BaseClient):
    def __init__(self, config: Any, on_data_callback: Any = None, on_error_callback: Any = None) -> None:
        """Initialize the DC charger client and its register parsers."""
    ...
    def parse_device_info(self, bs: bytes | bytearray) -> None:
        """Parse DC charger device identification fields from a response."""
        data = {}
    ...
    def parse_device_address(self, bs: bytes | bytearray) -> None:
        """Parse and store the DC charger device address."""
        data = {}
        data['device_id'] = bytes_to_int(bs, 4, 1)
        self.data.update(data)

    def parse_charging_info(self, bs: bytes | bytearray) -> None:
        """Parse charging voltage, current, and power telemetry."""
        data = {}
    ...
    def parse_state(self, bs: bytes | bytearray) -> None:
        """Parse the charger operating state flags."""
        data = {}
        alarms = {}
    ...
    def parse_battery_type(self, bs: bytes | bytearray) -> None:
        """Parse the configured battery type identifier."""
        data = {}
        data['function'] = FUNCTION.get(bytes_to_int(bs, 1, 1))
